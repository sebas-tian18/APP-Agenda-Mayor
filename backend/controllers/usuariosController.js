...
class UsuariosController {
    ...
    async HallarAdultoMayor(req, res) {
        const { id } = req.params;
        const [rows] = await db.promise().query('SELECT * FROM adulto_mayor where id_usuario = ?', [id]);
    ...
    async HallarCitaAdultoMayor(req, res) {
        const { id } = req.params;
        const [rows] = await db.promise().query('SELECT * FROM cita where id_adulto_mayor = ?', [id]);

        if (rows.length === 0) {
            throw errors.NotFoundError('Cita no encontrado');
        }
